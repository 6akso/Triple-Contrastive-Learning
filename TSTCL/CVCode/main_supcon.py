...
from TSI_CL_losses_TYPE1 import Fourdloss
...
def parse_option():
    ...
    parser.add_argument('--epochs', type=int, default=1000,help='number of training epochs') #1000
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        if opt.method == 'SupCon':
            fourdloss=Fourdloss()
            loss2= fourdloss.forward(features,labels)
...
def main():
    ...
    # 配置日志记录器
    logging.basicConfig(filename='train_{}.log'.format(timestamp), level=logging.INFO)
    opt = parse_option()
    ...
    # save the last model
    save_file = os.path.join(opt.save_folder, 'last_{}.pth'.format(timestamp))
