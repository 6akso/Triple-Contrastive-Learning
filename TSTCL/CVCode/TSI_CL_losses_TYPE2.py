...
class Fourdloss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        for i in range(mask2.size(0)):
            ...
            # Identify negative samples for the current anchor
            negtive_mask = mask2[i]
            negtive_indices = torch.where(negtive_mask)[0]  # Get indices of negative samples
            negtive_features = contrast_feature[negtive_indices]

            if negtive_features.size(0) == 0:
                continue

            # Compute pairwise dot products among negative samples with different labels
            different_label_mask = (
                        labels2[negtive_indices].unsqueeze(1) != labels2[negtive_indices].unsqueeze(0)).float()
            dot_product_matrix = torch.matmul(negtive_features, negtive_features.T) / self.temperature
