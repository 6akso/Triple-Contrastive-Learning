...
from Tripe_Loss_Type2 import QuadrupletLoss2
...
class Instructor:
    ...
    def __init__(self, args, logger):
        ...
        if args.device.type == 'cuda':
            self.logger.info('> cuda memory allocated: {}'.format(torch.cuda.memory_allocated(args.device.index)))  #将GPU的内存使用情况记录到日志中
        self._print_args()
    ...
    def _train(self, dataloader, criterion, optimizer):
        ...
        for inputs, targets in tqdm(dataloader, disable=self.args.backend, ascii=' >='):
            ...
            loss1 = criterion(outputs1, targets)#初始的对比损失函数
            loss_init=loss1
            quadruplet=QuadrupletLoss2(0.5,0.05)
            loss3=quadruplet.forward(outputs1,targets)
            loss=loss_init+0.5*loss3
            #-------------------------------------------------------------------------------
            optimizer.zero_grad() #优化器梯度设置为0，
            loss.backward()
            optimizer.step()  #更新模型参数
            train_loss += loss.item() * targets.size(0)  #训练过程损失函数的值

            n_correct += ((torch.argmax(outputs1['predicts'], -1) == targets) ).sum().item()  #正确预测的数据的数量
            n_train += targets.size(0)  #训练数据的数量

        return train_loss / n_train, n_correct / n_train,loss_init,loss3

    def _test(self, dataloader, criterion):
        num_classes = args.num_classes # 获取数据集中的类别数
        test_loss, n_correct, n_test = 0, 0, 0
        n_tp = [0] * num_classes  # 初始化每个类别的 TP、FP、FN 的值为 0
        n_fp = [0] * num_classes
        n_fn = [0] * num_classes
        self.model.eval()
        with torch.no_grad():
            for inputs, targets in tqdm(dataloader, disable=self.args.backend, ascii=' >='):
                ...
                for c in range(num_classes):
                    n_tp[c] += ((predicted_labels == c) & (targets == c)).sum().item()  # 计算每个类别的 TP 的数量
                    n_fp[c] += ((predicted_labels == c) & (targets != c)).sum().item()  # 计算每个类别的 FP 的数量
                    n_fn[c] += ((predicted_labels != c) & (targets == c)).sum().item()  # 计算每个类别的 FN 的数量

        accuracy = n_correct / n_test
        class_precision = [n_tp[c] / (n_tp[c] + n_fp[c]) if (n_tp[c] + n_fp[c]) > 0 else 0 for c in range(num_classes)]  # 计算每个类别的 Precision
        class_recall = [n_tp[c] / (n_tp[c] + n_fn[c]) if (n_tp[c] + n_fn[c]) > 0 else 0 for c in range(num_classes)]  # 计算每个类别的 Recall
        class_f1_score = [2 * class_precision[c] * class_recall[c] / (class_precision[c] + class_recall[c]) if (class_precision[c] +class_recall[c]) > 0 else 0 for c in range(num_classes)]  # 计算每个类别的 F1 score
        # 计算 Precision、Recall 和 F1 score 的平均值
    ...
    def run(self):
        ...
        for fold, (train_indices, val_indices) in enumerate(kf.split(dataset)):
            ...
            train_dataloader = torch.utils.data.DataLoader(train_sampler, batch_size=self.args.train_batch_size,  num_workers=workers,collate_fn=collate_fn, pin_memory=True,shuffle=True)
            test_dataloader = torch.utils.data.DataLoader(val_sampler, batch_size=self.args.test_batch_size, num_workers=workers,collate_fn=collate_fn, pin_memory=True,shuffle=True)
            ...
            with open("metrics.txt", "w") as f:
                ...
                for epoch in range(self.args.num_epoch):
                    train_loss, train_acc,loss_init,loss_init2= self._train(train_dataloader, criterion, optimizer)
                    test_loss, test_acc,precision, recall, f1_score = self._test(test_dataloader, criterion)
                    ...
                    self.logger.info('[train] loss: {:.4f}, acc: {:.2f},loss_init:{:.4f},loss_init2:{:7f}'.format(train_loss, train_acc*100,loss_init,loss_init2))
                ...
                plt.plot(range(self.args.num_epoch), trainlosses, label="trainLoss")
                plt.plot(range(self.args.num_epoch), testlosses, label="testLoss")
                plt.plot(range(self.args.num_epoch), trainaaccuracy, label="trainaccuracy")
                plt.plot(range(self.args.num_epoch), testaccuracy, label="testaccuracy")
                plt.xlabel("Epoch")
                plt.ylabel("Loss")
